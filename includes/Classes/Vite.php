...
class Vite {
	public static function base_path() {
		return TODO_PLUGIN_URL . 'assets/dist/';
	}
	...
	private static function cssTag( string $entry ): string {
		...
		foreach ( self::cssUrls( $entry ) as $key => $url ) {
			wp_register_style( "catfolders/$key", $url, array(), TODO_VERSION );
			wp_enqueue_style( "catfolders/$key", $url, array(), TODO_VERSION );
		}
		return $tags;
	}
	...
	private static function getManifest(): array {
		...
		$content = file_get_contents( TODO_PLUGIN_DIR . 'assets/dist/manifest.json' );
	...
	private static function getPublicURLBase() {
		return TODO_PLUGIN_DEVELOPMENT ? '/dist/' : self::base_path();
	}
